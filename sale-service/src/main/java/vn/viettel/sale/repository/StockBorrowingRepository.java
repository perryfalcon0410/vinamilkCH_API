...
public interface StockBorrowingRepository extends BaseRepository<StockBorrowing> {
    @Query(value = "SELECT * FROM STOCK_BORROWING WHERE TO_SHOP_ID =:toShopId AND STATUS_IMPORT = 1 AND STATUS = 1 ", nativeQuery = true)
    List<StockBorrowing> getStockBorrowing(Long toShopId);

    @Query(value = "SELECT * FROM STOCK_BORROWING WHERE SHOP_ID =:shopId AND STATUS_EXPORT = 1 AND STATUS = 1 ", nativeQuery = true)
