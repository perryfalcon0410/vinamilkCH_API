...
public class SalePromotionServiceImpl extends BaseServiceImpl<SaleOrder, SaleOrderRepository> implements SalePromotionService {
    ...
    public SalePromotionCalculationDTO getSaleItemPromotions(OrderPromotionRequest request, Long shopId,
        ...
        for (PromotionProgramDTO program: programs) {
            ...
            switch (PromotionProgramType.valueOf(program.getType())) {
                ...
                case ZV17:
                case ZV18:
                    this.addItemPromotion(results, this.getAutoItemPromotionZV01ToZV21(program, orderData, shopId, customerType.getWareHouseTypeId(), 0,0,0,0, forSaving));
                    break;
                case ZV19:
                case ZV20:
                case ZV21:
                ...
                case ZM:
                    ...
                    else this.addItemPromotion(results, this.getItemPromotionZM(program, orderData, shopId, customerType.getWareHouseTypeId(), amount,
        ...
        for(Map.Entry<PromotionProgramDTO, Boolean> entry: mapZV192021.entrySet()){
            if(entry.getValue() == null || entry.getValue()){
                SalePromotionDTO item = this.getAutoItemPromotionZV01ToZV21(entry.getKey(), orderData, shopId, customerType.getWareHouseTypeId(),
        ...
        for (PromotionProgramDTO programDTO : lstZV23){
            SalePromotionDTO item = this.getItemPromotionZV23(programDTO, orderData, shopId, customerType.getWareHouseTypeId(), request.getCustomerId(),
        ...
        for (Map.Entry<PromotionProgramDTO, Double> entry : lstZmType3.entrySet()){
            SalePromotionDTO item = this.getItemPromotionZM(entry.getKey(), orderData, shopId, customerType.getWareHouseTypeId(), entry.getValue(),
    ...
    private SalePromotionDTO getItemPromotionZM(PromotionProgramDTO program, ProductOrderDataDTO orderData, Long shopId,  Long warehouseId,
        ...
        if (flag/* || ( orderData == null && program.getGivenType() != null && program.getGivenType() == 1)*/){
            ...
            if (program.getGivenType() != null && program.getGivenType() == 1){// tặng sản phẩm
                ...
                List<PromotionProgramDiscountDTO> programDiscounts = promotionClient.findPromotionDiscountByPromotion(program.getId()).getData();
    ...
    private SalePromotionDTO calZMAmount(PromotionProgramDTO program, ProductOrderDataDTO orderData, Long shopId, List<PromotionProgramDiscountDTO> discountDTOs, double amountProInTax,
        ...
        if(program.getGivenType() != null && program.getGivenType() == 3){
            totalAmountInTax = totalAmountInTax - amountProInTax;
            totalAmountExtax = totalAmountExtax - amountProExTax;
        }
        List<PromotionProgramDiscountDTO> discountNeedUpdate = new ArrayList<>();

        //sort sử dụng giảm giá % trước sau đến giảm giá tiền, sau đến nhập tay
        List<PromotionProgramDiscountDTO> discountItems = new ArrayList<>();
        List<PromotionProgramDiscountDTO> itemPercents = discountDTOs.stream().filter(p -> p.getDiscountPercent()!=null).collect(Collectors.toList());
        Collections.sort(itemPercents,  Comparator.comparing(PromotionProgramDiscountDTO::getDiscountPercent).reversed());

        List<PromotionProgramDiscountDTO> itemAmounts =  discountDTOs.stream().filter(p -> p.getDiscountAmount()!=null).collect(Collectors.toList());
        Collections.sort(itemAmounts, Comparator.comparing(PromotionProgramDiscountDTO::getDiscountAmount).reversed());

        List<PromotionProgramDiscountDTO> itemMaxes = discountDTOs.stream().filter(p -> p.getMaxDiscountAmount()!=null).collect(Collectors.toList());
        Collections.sort(itemMaxes, Comparator.comparing(PromotionProgramDiscountDTO::getMaxDiscountAmount).reversed());

        discountItems.addAll(itemPercents);
        discountItems.addAll(itemAmounts);
        discountItems.addAll(itemMaxes);

        Double amtInTax = 0D;
        Double amtExTax = 0D;
        double maxAmount = 0;

        for(PromotionProgramDiscountDTO discountDTO : discountItems){
            /*
            ...
            if ( discountDTO.getMinSaleAmount() != null && discountDTO.getMinSaleAmount() <= compareAmt) {//đạt số tiền tối thiểu đơn hàng cần đạt
                double amountIn = 0;
                double amountEx = 0;
                if (discountDTO.getDiscountAmount() != null && discountDTO.getDiscountAmount() > 0) { //km tiền
                    amountIn = discountDTO.getDiscountAmount();
                    amountEx = amountIn / ((100 + billPercent) / 100);

                    if (!isInclusiveTax) {
                        amountEx = discountDTO.getDiscountAmount();
                        amountIn = amountEx * ((100 + billPercent) / 100);
                    }
                    ...
                    if (discountDTO.getMaxSaleAmount() != null && discountDTO.getMaxSaleAmount() > 0 && discountDTO.getMaxSaleAmount() < compareAmt) {
                        ...
                        amtSaleExTax = amtSaleInTax / ((100 + billPercent) / 100);
                        if (!isInclusiveTax) {
                            amtSaleExTax = discountDTO.getMaxSaleAmount();
                            amtSaleInTax = amtSaleExTax * ((100 + billPercent) / 100);
                        }
                    }
                    amountIn = amtSaleInTax * discountDTO.getDiscountPercent() / 100;
                    amountEx = amtSaleExTax * discountDTO.getDiscountPercent() / 100;
                    compareAmt = amountIn;
                    if (!isInclusiveTax) compareAmt = amountEx;
                    ...
                    if (discountDTO.getMaxDiscountAmount() != null && discountDTO.getMaxDiscountAmount() > 0 && compareAmt > discountDTO.getMaxDiscountAmount()) {
                        amountIn = discountDTO.getMaxDiscountAmount();
                        amountEx = amountIn / ((100 + billPercent) / 100);
                        if (!isInclusiveTax) {
                            amountEx = discountDTO.getMaxDiscountAmount();
                            amountIn = amountEx * ((100 + billPercent) / 100);
                        }
                    }
                    ...
                    if (inputAmount != null && inputAmount > 0) {
                        amtInTax = inputAmount;
                        amtExTax = amtInTax / ((100 + billPercent) / 100);
                        if (!isInclusiveTax) {
                            amtExTax = inputAmount;
                            amtInTax = amtExTax * ((100 + billPercent) / 100);
                        }
                    }else amtInTax = null;
                    maxAmount = discountDTO.getMaxDiscountAmount();

                    break;
                }
                /*
                    kiểm tra nếu là giảm giá khách hàng given type = 3
                    Nếu tổng tiền đơn hàng - tiền giảm giá < tiền km => thì ko sử dụng giảm giá khách hàng
                */
                if(program.getGivenType() != null && program.getGivenType() == 3){
                    ...
                    totalAmountInTax = totalAmountInTax - amountIn;
                    totalAmountExtax = totalAmountExtax - amountEx;
                    amtInTax += amountIn;
                    amtExTax += amountEx;
        ...
        SalePromotionDTO salePromotion = null;
        //trường hợp amtInTax == null -> cho nhập giá trị
        if(amtInTax == null || amtInTax > 0) {
            if(amtInTax == null){
                amtInTax = 0D;
                amtExTax = 0D;
            }
            ...
            if (forSaving) {
                if(amtInTax < 1) spDto.setPercentage(0D);
                else  spDto.setPercentage((amtInTax - amtExTax )/ amtExTax * 100);
                spDto.setMaxAmount(spDto.getAmount());

                double percentInTax = calPercent(orderData.getTotalPrice(), salePromotion.getTotalAmtInTax());
                double percentExTax = calPercent(orderData.getTotalPriceNotVAT(), salePromotion.getTotalAmtExTax());
                ...
                salePromotion.setDiscountDTOs(discountNeedUpdate);
            }
        ...
        if (percent > 0 && amountInTax > 0){
            ...
            if(forSaving) {
                ...
                List<SaleDiscountSaveDTO> saveInfo = initSaleDiscountSaveDTO(new ArrayList<>(lstProductHasPromotion.values()), 1,
                        ppInTax, ppExTax, isInclusiveTax(program.getDiscountPriceType()), salePromotion.getTotalAmtInTax(), salePromotion.getTotalAmtExTax());
    ...
    private HashMap<Integer, Integer> getNextLevel(List<ProductOrderDetailDataDTO> productOrders, HashMap<Long, HashMap<Integer, List<PromotionProgramDetailDTO>>> mapOrderNumber,
                    boolean isInclusiveTax, Integer level, Integer currentLevel, int checkMulti, String type, HashMap<Integer, Integer> lstLv){
            ...
            if(mapOrderNumber.containsKey(productOrder.getProductId())) {
                ...
                for (Integer lv : lstLevel) { // số thấp nhất là mức cao nhất
                    if(level == null || level <= lv) {
                        if(lstLv != null && lstLv.size() > 0 && !lstLv.containsKey(lv) && currentLevel > lv) continue;
    ...
    private SalePromotionDTO getZV13ToZV18(PromotionProgramDTO program, ProductOrderDataDTO orderData, Long shopId, Long warehouseId, boolean forSaving) {
        ...
        for (Integer lv : lstLevel) {
            if(level <= lv) {
                HashMap<Integer, Integer> newLv = getNextLevel(orderData.getProducts(),mapOrderNumber,isInclusiveTax, level, lv, checkMulti, type, lstLv);
                if(newLv != null) {
                ...
                for (ProductOrderDetailDataDTO productOrder : orderData.getProducts()){
                    if(mapOrderNumber.containsKey(productOrder.getProductId())) {
                        ...
                        if("zv13".equalsIgnoreCase(type) || "zv14".equalsIgnoreCase(type)){ // km %
                            amtOrderInTax = discountItem.getSaleQty() * productOrder.getPrice() * multi;
                            amtOrderExTax = discountItem.getSaleQty() * productOrder.getPriceNotVAT() * multi;
                        }else if("zv16".equalsIgnoreCase(type) || "zv17".equalsIgnoreCase(type)) { // km tiền
                            double productPercent = (productOrder.getPrice() - productOrder.getPriceNotVAT() )/ productOrder.getPriceNotVAT() * 100;
                            if(isInclusiveTax) {
                                amtOrderInTax = discountItem.getSaleAmt() * multi;
                                amtOrderExTax = discountItem.getSaleAmt() / (( 100 + productPercent ) / 100) * multi;
                            }else{
                                amtOrderInTax = discountItem.getSaleAmt() * (( 100 + productPercent ) / 100) * multi;
                                amtOrderExTax = discountItem.getSaleAmt() * multi;
                            }
                        }
                        amountOrderInTax += amtOrderInTax;
                        amountOrderExTax += amtOrderExTax;
        ...
        if ("zv07".equalsIgnoreCase(type) // Mua 1 nhóm sản phẩm nào đó - với số lượng xác định (tổng), thì được giảm % tổng tiền
            ...
            if(defaultItem.getDisPer() != null && defaultItem.getDisPer() > 0) {
                ...
                if (forSaving) {
                    discountDTO.setPercentage(percent);
                    discountDTO.setMaxAmount(discountDTO.getAmount());
                    List<SaleDiscountSaveDTO> saveInfo = initSaleDiscountSaveDTO(lstProductHasPromotion, level, percent, percent, isInclusiveTax,
            ...
            if(defaultItem.getDiscAmt() != null && defaultItem.getDiscAmt() > 0) {
                ...
                for (Map.Entry<Integer, Integer> entry : sortedLstLv.entrySet()) {
                    ...
                    discountExTax += amtExTax;
                    if (forSaving) {
                        double percentInTax = calPercent(totalOrderAmtInTax, amtInTax);
                        double percentExTax = calPercent(totalOrderAmtExtax, amtExTax);
                        List<SaleDiscountSaveDTO> infos = initSaleDiscountSaveDTO(lstProductHasPromotion, entry.getKey(), percentInTax, percentExTax, isInclusiveTax,
                                amtInTax, amtExTax);
    ...
    private List<SaleDiscountSaveDTO> initSaleDiscountSaveDTO(List<ProductOrderDetailDataDTO> products, Integer level, double percentInTax, double percentExTax,
