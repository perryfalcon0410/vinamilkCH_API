...
public class RestAdvice implements ResponseBodyAdvice {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        ...
        if (requestConverted.getAttribute(Constants.REQUEST_SECRET_MARK_AS_OUTSIDE_REQUEST)!=null) {
            boolean isOutsideClient =
                    (boolean) requestConverted.getAttribute(Constants.REQUEST_SECRET_MARK_AS_OUTSIDE_REQUEST);
            if (isOutsideClient) {
                Claims claims = (Claims) requestConverted.getAttribute(Constants.REQUEST_SECRET_MARK_AS_CLAIM_OUTSIDE_REQUEST);
                ...
                String token = (String) requestConverted.getAttribute(Constants.REQUEST_SECRET_MARK_AS_OLD_TOKEN);
