...
public class SalesCategoryExcel {
    ...
    private void writeHeaderLine() {
        sheet = workbook.createSheet("Doanh số theo ngành hàng");
        ////////// HEADER /////////////////////////////
        ...
        createCell(row, 3, "ĐỊA CHỈ", colNameStyle);
        createCell(row, 4, "TẦN SUẤT", colNameStyle);
        createCell(row, 5, "THANH TOÁN", colNameStyle);
        ...
        if (!saleCate.isEmpty()) {
            int start = 9;
            for (int i = 0; i < saleCate.size(); i++) {
                Row rowContent = sheet.createRow(start);
                SalesByCategoryReportDTO record = saleCate.get(i);
                createCell(rowContent, 0, i + 1, dataStyle);
                createCell(rowContent, 1, record.getCustomerCode(), dataStyle);
                createCell(rowContent, 2, record.getCustomerName(), dataStyle);
                createCell(rowContent, 3, record.getCustomerAddress(), dataStyle);
                createCell(rowContent, 4, record.getOrderNumber(), dataStyle);
                createCell(rowContent, 5, record.getTotal(), dataStyle2);
                start++;
            }
        }
    }
